...
- RigorPilot Explore 现在已经接通：bounded idea seed generation、显式 idea score breakdown、atomic idea decomposition、以及 planned / heuristic / observed 分层的 implementation fidelity。
...
## 🧠 RigorPilot Explore 流程

`ai-research-explore` 是 RigorPilot Explore 入口：研究者已经冻结 task family、dataset、evaluation method，并且给出了 SOTA 参考；随后显式授权 AI 在 `current_research` 上进行受约束、可审计、candidate-only 的探索。这是 meaningful and potentially novel 的候选研究工作，不是已经验证的 novelty。
...
双循环只是工作节奏，不是 never-stop 自治代理。遇到明确 blocker、科学含义不清、预算耗尽、缺少 anchor/evaluation 或需要人工 checkpoint 时应停止。RigorPilot Explore 不得宣称 trusted reproduction success、完整 benchmark 结论、或已经验证的 novelty。
...
| Explore | `ai-research-explore` | 在 `current_research` 上运行 RigorPilot Explore 编排，负责 idea gate 与 experiment governance |
...
| `explore_outputs/` | RigorPilot Explore changeset、idea gate、experiment plan、experiment manifest、scientific changelog、comparability report、split static/runtime smoke reporting、ledger、top runs summary |
...
- `ai-research-explore` 是受治理的 RigorPilot Explore orchestrator，不是开放式 autonomous research agent。
