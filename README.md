...
| Signal | What to remember |
|---|---|
| 🧭 Purpose | Research-first Agent Skills for deep learning experiments. |
| 🔒 Default path | Ambiguous requests go to the trusted lane: reproduce, set up, run, train, analyze, or debug without silent scientific drift. |
| 🧪 Explicit path | Exploration only starts when the researcher asks for candidate-only work on top of `current_research`. |
| 📦 Evidence | Outputs are durable bundles such as `repro_outputs/`, `analysis_outputs/`, `train_outputs/`, `debug_outputs/`, and `explore_outputs/`. |
| 🔗 Clients | The canonical contract is `SKILL.md`; Codex, Claude Code, and neutral Agent Skills installs are supported. |

## 🚀 Start Fast

Most users only need one of these entrypoints:

| Goal | Install command |
|---|---|
| Full RigorPilot skill set | `npx skills add lllllllama/rigorpilot-skills --all` |
| Trusted README-first reproduction | `npx skills add lllllllama/rigorpilot-skills --skill ai-research-reproduction` |
| Explicit candidate-only exploration | `npx skills add lllllllama/rigorpilot-skills --skill ai-research-explore` |

After installing, use the selection table below if you are unsure which skill
matches the task.
